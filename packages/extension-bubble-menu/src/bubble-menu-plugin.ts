...
import { isTextSelection, posToDOMRect } from "@blockslides/core";
import type {
  EditorState,
  PluginView,
  Transaction,
} from "@blockslides/pm/state";
import { NodeSelection, Plugin, PluginKey } from "@blockslides/pm/state";
...
export class BubbleMenuView implements PluginView {
  ...
  show() {
    ...
    const appendToElement =
      typeof this.appendTo === "function" ? this.appendTo() : this.appendTo;
