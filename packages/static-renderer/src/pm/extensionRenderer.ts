...
import type { DOMOutputSpec, Mark } from '@blockslides/pm/model'
...
export function mapNodeExtensionToReactNode<T>(
  ...
  if (!renderToHTML) {
    ...
    return [
      ...
      () => {
        throw new Error(
          `[blockslides error] Node ${extension.name} cannot be rendered, it is missing a "renderToHTML" method, please implement it or override the corresponding "nodeMapping" method to have a custom rendering`,
        )
      },
    ]
  }
