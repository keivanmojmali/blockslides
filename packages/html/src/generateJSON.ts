import type { Extensions } from '@blockslides/core'
import { getSchema } from '@blockslides/core'
import type { ParseOptions } from '@blockslides/pm/model'
import { DOMParser } from '@blockslides/pm/model'
...
export function generateJSON(html: string, extensions: Extensions, options?: ParseOptions): Record<string, any> {
  if (typeof window === 'undefined') {
    throw new Error(
      'generateJSON can only be used in a browser environment\nIf you want to use this in a Node environment, use the `@blockslides/html/server` import instead.',
    )
  }
