...
import type { Mappable, Transform } from "@blockslides/pm/transform";
import type {
  Decoration,
  DecorationAttrs,
  EditorProps,
...
export type ExtensionAttribute = {
  type: string;
  name: string;
  attribute: Required<Omit<Attribute, "validate">> &
    Pick<Attribute, "validate">;
};
...
export type MarkViewRenderer<Props = MarkViewRendererProps> = (
  props: Props
) => MarkView;
...
export type MarkdownHelpers = {
  ...
  renderChildren: (
    node: Node[] | Node,
    ctxOrSeparator?: RenderContext | string
  ) => string;
