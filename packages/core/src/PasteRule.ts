...
import { Fragment } from "@blockslides/pm/model";
import type { EditorState } from "@blockslides/pm/state";
import { Plugin } from "@blockslides/pm/state";
  ...
  return matches.map((pasteRuleMatch) => {
    ...
    if (pasteRuleMatch.replaceWith) {
      if (!pasteRuleMatch.text.includes(pasteRuleMatch.replaceWith)) {
        console.warn(
          '[blockslides warn]: "pasteRuleMatch.replaceWith" must be part of "pasteRuleMatch.text".'
        );
      }
...
export function pasteRulesPlugin(props: {
  ...
  let pasteEvent =
    typeof ClipboardEvent !== "undefined" ? new ClipboardEvent("paste") : null;
  ...
  const processEvent = ({
    ...
    try {
      dropEvent =
        typeof DragEvent !== "undefined" ? new DragEvent("drop") : null;
    } catch {
      dropEvent = null;
    }
    pasteEvent =
