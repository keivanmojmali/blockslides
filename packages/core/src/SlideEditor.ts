...
import type {
  ...
  NodeType,
  Schema,
} from "@blockslides/pm/model";
import type { Plugin, PluginKey, Transaction } from "@blockslides/pm/state";
import { EditorState } from "@blockslides/pm/state";
import { EditorView } from "@blockslides/pm/view";
import type { Step } from "@blockslides/pm/transform";
...
export class SlideEditor extends EventEmitter<EditorEvents> {
    ...
    return new Proxy(
      ...
      } as EditorView,
      {
        get: (obj, key) => {
          ...
          throw new Error(
            `[blockslides error] The editor view is not available. Cannot access view['${
              key as string
  ...
  private createDoc(): ProseMirrorNode {
    ...
    try {
      ...
      if (
        !(e instanceof Error) ||
        ![
          "[blockslides error] Invalid JSON content",
          "[blockslides error] Invalid HTML content",
        ].includes(e.message)
      ) {
  ...
  public prependClass(): void {
    this.view.dom.className = `blockslides-editor ${this.view.dom.className}`;
  }
  ...
  private dispatchTransaction(transaction: Transaction): void {
    ...
    if (this.isCapturingTransaction) {
      ...
      transaction.steps.forEach((step: Step) => this.capturedTransaction?.step(step));
    ...
    if (
      ...
      !transactions.some((tr: Transaction) => tr.docChanged) ||
