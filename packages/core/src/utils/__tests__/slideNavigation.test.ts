...
describe('Slide Navigation', () => {
  ...
  describe('transitions', () => {
    ...
    it('should apply transition classes to current and target slides', (done) => {
      ...
      expect(slides[0].classList.contains('transition-fade-out') || 
             slides[0].className.includes('fade')).toBeTruthy();
      
      // Advance through first timeout
      jest.advanceTimersByTime(100);
      
      // Check that enter class is applied to next slide
      expect(slides[1].classList.contains('transition-fade-in') ||
             slides[1].className.includes('fade')).toBeTruthy();
