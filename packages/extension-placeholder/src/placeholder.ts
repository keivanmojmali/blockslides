      ...
      new Plugin({
        ...
        props: {
          decorations: (state: EditorState) => {
            ...
            const { showOnlyWhenEditable, showOnlyCurrent, includeChildren } = this.options
            ...
            doc.descendants((node: ProseMirrorNode, pos: number) => {
              ...
              const hasAnchor = selection.$anchor.pos >= pos && selection.$anchor.pos <= pos + node.nodeSize
              const isEmpty = !node.isLeaf && isNodeEmpty(node, includeChildren)
