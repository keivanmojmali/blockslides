...
export interface LayoutPickerOptions {
   ...
   */
  onLayoutSelect: ((layoutId: string, slideElement: HTMLElement | null) => void) | null
}
 ...
 */
function applySlideLayoutDirect(view: EditorView, slideOffset: number, layout: string): void {
  try {
    ...
    // Replace the slide
    const tr = state.tr.replaceRangeWith(slidePos, slidePos + slideNode.nodeSize, newSlide)
...
function createPlaceholderContent(_layout: string, schema: any): ProseMirrorNode[] {
  // Placeholder implementation
