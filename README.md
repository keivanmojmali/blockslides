...
```
pnpm add @autoartifacts/react @autoartifacts/core @autoartifacts/pm @autoartifacts/extension-kit
```

__Step 2 - Create useSlideEditor hook__
```
"use client";
...
```
__Step 3 - Create SlideEditor Component__ 
```
...
```
__Step 4 - Feed the editor content__
...
All nodes and marks follow standard ProseMirror specs, so you can extend or override them as needed. The `@autoartifacts/extension-kit` package exports a curated set of extensions (slides, rows, columns, text blocks, list support, layout UI helpers) to jump-start editor configuration. __All extensions are set to true by default__
